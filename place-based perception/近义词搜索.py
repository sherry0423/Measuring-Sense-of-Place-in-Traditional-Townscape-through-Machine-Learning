...
class SynonymDetector:
    def __init__(self, list_a, category_mapping):
    ...
    def find_match(self, word_b, category):
        emb_b = self.model.encode(word_b, convert_to_tensor=True)
        relevant_indices = [i for i, cat in enumerate(self.category_mapping) if cat == category]
        relevant_embeddings = self.embeddings_a[relevant_indices]
        relevant_words = [self.category_mapping[i] for i in relevant_indices]
        ...
        return match_word, round(float(max_score), 3)
...
def main():
    ...
    # 初始化检测器
    detector = SynonymDetector(list_a, categories_b)
    results = []
    ...
    # 批量处理
    for word, category in zip(list_b, categories_b):
        match_word, score = detector.find_match(word, category)
        results.append({
            "目标词": word,
            "匹配词": match_word,
            "相似度": score,
