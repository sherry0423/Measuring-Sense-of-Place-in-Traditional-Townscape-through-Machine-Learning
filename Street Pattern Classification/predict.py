...
class PREDICT(Build_model):
    ...
    def Predict(self):
        ...
        for img_id, img in enumerate(data_list):
            ...
            class_list = self.classes_id()


            jiangnan_idx = class_list.index("jiangnan_water_towns")  # 正确的类别索引


            pred_idx = np.argmax(pred)

            if pred_idx == jiangnan_idx:
                Prob_W = confidence
            else:
                Prob_W = 1 - confidence
            print('Prob_W is: ', Prob_W)
